...
def _pack(pairs):
    ...
    for i, (c, seg) in enumerate(pairs):
        ...
        tp[i] = [p["x0"], p["vx0"], p["ax"], p["y0"], p["vy0"], p["ay"],
                 p["z0"], p["vz0"], p["az"]]
        # rel is never NaN here: a release-less clip produces no candidate runs
        rel = poselib.derive_release(c["df"])
        t_lo, t_hi = -rel - poselib.RELEASE_HALF_S, -rel + poselib.RELEASE_HALF_S
...
def solve_game(args):
    ...
    for c, f, q in zip(clips, fits, _ptz(clips, C)):
        ...
        rows.append({"play_id": c["play_id"], "game_pk": game_pk, "park": c["park"],
...
if __name__ == "__main__":
    ...
    want = set(games)
    zones_by = {game_pk: g for game_pk, g in zones.groupby("game_pk") if game_pk in want}
