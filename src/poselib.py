...
What lives here:
  ...
  derive_release:            release time (s): the clip's release hint, or NaN when
                             it has none (a release-less clip drops)
  ball_runs:                 one clip's ball detections, split into contiguous runs;
                             candidate_runs starts here
...
RELEASE_HALF_S = 0.60   # run-selection and t0 band around the clip's release
...
def derive_release(df):
    """Release time (s) for glove-window placement."""
...
def candidate_runs(df, fps, T_plate):
    ...
    if len(ball) < 8:
        return []
    rel = derive_release(df)
    if np.isnan(rel):
        return []
    lo, hi = rel - RELEASE_HALF_S, rel + RELEASE_HALF_S
...
def load_game(game_pk, pbp, zones, tracks_dir, zone_y_ft):
    ...
    by_play = dict(tuple(tracks.groupby("play_id")))
    boxes = zones[zones["game_pk"] == game_pk].dropna(subset=["x_left", "y_top", "x_right", "y_bottom"])
    pbp = pbp[pbp["game_pk"] == game_pk].set_index("play_id")
    clips = []
    for _, box in boxes.iterrows():
        play_id = box["play_id"]
        if play_id not in pbp.index:
            continue
        p = pbp.loc[play_id]
        df = by_play.get(play_id)
        if df is None:
            continue
        ...
        clips.append({"play_id": play_id, "game_pk": int(p["game_pk"]), "park": p["home_team"], "p": p,
...
def load_season(year):
    ...
    assert {"game_pk", "play_id", "x_left", "y_top", "x_right", "y_bottom"} <= set(zones.columns), \
