...
Writes:     data/<year>/glove_locations/<game_pk>.csv.gz, one row per detection:
            game_pk, play_id, frame_idx, x_in, z_in, y_depth_ft, fps, release_s
...
def glove_pixels(df):
    ...
    return df[df["glove_center_x"].notna()][["frame_idx", "glove_center_x", "glove_center_y"]]
...
def locate_game(args):
    ...
    parts = []
    for play_id, df in tracks.groupby("play_id", sort=True):
        if play_id not in poses.index:
            continue
        pose = poses.loc[play_id][["Cx", "Cy", "Cz", "pan", "tilt", "roll", "f_px"]].to_numpy(float)
        fps = float(df["fps"].iloc[0])
        release_s = poselib.derive_release(df)
        ...
        xz.insert(0, "play_id", play_id)
        xz.insert(0, "game_pk", game_pk)
    ...
    return df["play_id"].nunique(), len(df)


if __name__ == "__main__":
    year = sys.argv[1] if len(sys.argv) > 1 else "2026"
    base = poselib.DATA / year
    poses = pd.read_csv(base / "camera_poses.csv.gz").set_index("play_id")
